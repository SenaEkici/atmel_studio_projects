...
static uint8_t dest_mac_address[6] = {0x64,0x51,0x06,0xA8,0x09,0xA2};
	//static uint8_t dest_mac_address[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
static PROGMEM *client_additionalheaderline;
static char *client_postval;
static PROGMEM *client_urlbuf;
static char *client_urlbuf_var;
static PROGMEM *client_hoststr;
...
//const char arpreqhdr[] PROGMEM ={0XFF,0XFF,0XFF,0XFF,0XFF,0XFF,0XFF,0XFF};	
...
#endif // NTP_client || WOL_client || UDP_client || TCP_client
...
//
void client_browse_url(PROGMEM *urlbuf, char *urlbuf_varpart, PROGMEM *hoststr,void (*callback)(uint8_t,uint16_t,uint16_t))
{
...
void client_http_post(PROGMEM *urlbuf, PROGMEM *hoststr, PROGMEM *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t))
{
        client_urlbuf=urlbuf;
        client_hoststr=hoststr;
...
void enc28j60gratuitousARPrequest( uint8_t* macaddr)
{
	...
	while (i < 4)
	{
		enc28j60WriteOp(ENC28J60_WRITE_BUF_MEM, 0, ipaddr[i]);
		i++;
	}
	// target data:
	i=0;
