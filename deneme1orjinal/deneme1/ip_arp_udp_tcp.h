...
extern void enc28j60gratuitousARPrequest( uint8_t* macaddr);
#endif
...
#ifdef WWW_client
// ----- http get
extern void client_browse_url(PROGMEM *urlbuf, char *urlbuf_varpart, PROGMEM *hoststr,void (*callback)(uint8_t,uint16_t,uint16_t));
...
extern void client_http_post(PROGMEM *urlbuf, PROGMEM *hoststr, PROGMEM *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t));
// The callback is a reference to a function which must look like this:
// void browserresult_callback(uint8_t statuscode,uint16_t datapos,uint16_t len)
// statuscode=0 means a good webpage was received, with http code 200 OK
// statuscode=1 an http error was received
